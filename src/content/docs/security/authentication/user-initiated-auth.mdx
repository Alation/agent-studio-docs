...
For detailed step-by-step instructions for each platform, refer to their individual documentation pages in the [MCP Client Setup guides](/agent-studio-docs/integrations/mcp-clients/claude/).
:::
...
- **Snowflake (Cortex Agents MCP connector):**
  ```
  https://identity.snowflake.com/oauth2/callback
  ```
  Plus your deployment's Snowsight URI: run `SELECT SYSTEM$ALLOWLIST();` in Snowflake, take the `SNOWSIGHT_DEPLOYMENT` entry starting with `apps-api`, and prefix it with `https://`.
...
ChatGPT does not yet support PKCE — disable the **PKCE Required** option when creating an OAuth client for it.
MS Copilot Studio supports and requires PKCE; leave the option enabled.
:::
