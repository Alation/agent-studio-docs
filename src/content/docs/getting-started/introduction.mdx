...
1. **Tools (the building blocks)**  
   We provide tools that can be accessed via **MCP** servers. You can use these tools to build agents in whatever platform you like—inside Alation or outside of it. A complete list of Alation-provided tools is available [here](/reference/tools/index).

2. **Out-of-the-box agents**  
   We provide pre-built agents that solve common tasks on top of the Alation catalog. These agents can be used as-is or serve as starting points for further agent customization. They can be accessed externally via **MCP** or **REST APIs**. A complete list of Alation-provided base agents is available [here](/reference/agents/index).
