...
<Aside type="tip">
Label names are case-sensitive — `"Confidential"` and `"confidential"` are treated as different keys.
Use the exact label name configured in `sensitivity_label_mappings`.
</Aside>

#### Exchange Online transport rules

By default, Exchange Online honors the `msip_labels` header from authenticated senders.
Check with your Microsoft 365 admin to confirm that transport rules are not configured to strip custom headers from inbound messages.
