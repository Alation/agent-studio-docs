...
- **OpenAPI documentation** at:
  ...
  Navigate to **OAuth Client Administration** → **POST /integration/core/v1/clients/** to register a new client.
  ...
  **Example payload for a confidential client:**
  ...
  {
    "name": "My Application",
    "redirect_uris": ["https://your-app.com/oauth/callback"], // Replace these with your application's redirect URI(s)
...
You'll need the **Redirect URI** (sometimes labeled **Callback URI**) from the third-party application you're building.
...
1. Create the `Client ID` and `Client Secret` without the `Redirect URI` first
2. Configure those credentials within Copilot Studio
3. Obtain the `Redirect URI` that Copilot Studio generates
4. Add that `Redirect URI` to the Alation configuration
