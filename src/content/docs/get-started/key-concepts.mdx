...
Agents are only as good as the knowledge you give them, so they need a source of information that is complete, governed, and current.
...
Agent Studio supports **OpenAI** and **Anthropic** models (including through AWS Bedrock), and you can [bring your own enterprise LLM](/agent-studio-docs/build/models/byom-setup/).
To choose the model behind your agents, see the [LLM configuration guide](/agent-studio-docs/build/models/llm-configuration/).
...
A complete list of these Alation-provided tools that are metered is available [here](/agent-studio-docs/build/tools/default-tools).
...
Metered actions are priced in Alation Consumption Units (ACUs). See [usage and metering](/agent-studio-docs/security/observability/usage/#how-consumption-is-priced) for the rate and how to track your consumption.
...
Alation provides both a hosted MCP server (the default) and a local one you can run with the Python SDK.
[Using the MCP server](/agent-studio-docs/integrations/mcp/using-the-mcp-server/#hosted-vs-local-servers) compares the two.
