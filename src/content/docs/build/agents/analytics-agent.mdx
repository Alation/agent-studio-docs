...
- `marketplace_id` (string): The ID of the marketplace to search for data products. If omitted, the agent uses the default marketplace.
...
**Key parameters:**
- `marketplace_id` (optional): The marketplace to search. When supplied as an agent input it is forwarded to the tool; otherwise the tool falls back to the default marketplace.
