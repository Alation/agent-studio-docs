...
8. **Publish the agent**
   - New agents start in **draft** status and are only visible to you and admin roles
   - When ready, click **Publish** in the top-right corner to make the agent visible to all users
   - See [Publishing and unpublishing agents](/agent-studio-docs/security/identity-and-permissions/#publishing-and-unpublishing-agents-in-the-ui) for details

<Aside type="tip" title="Existing custom agents">
Custom agents created before the publish/draft feature are now in **draft** status. Check the **Draft** tab and publish them to make them visible to other users again.
</Aside>
...
When you add or configure a tool, a modal displays all available parameters.
Each parameter has a **Scope** dropdown that lets you explicitly choose where the value comes from.
...
**Fixed input**
- The value is set once during agent configuration and cannot be changed at runtime
- Ideal for configuration that should never change between invocations
- Example: `marketplace_id` is a good candidate for fixed input when you want searches to always target a specific data product marketplace

**User input**
- The value is provided when the agent is invoked
- If the agent's input schema already has a field with the same name and a compatible type, it will automatically map to that field
- If no compatible field exists, the field is automatically added to the agent's input schema when you save
- Useful for parameters like `data_product_id` or user-defined scopes that vary per invocation

**Agent determined**
- The agent dynamically decides the value at runtime based on reasoning and context
- Appropriate for parameters that require inference, such as SQL queries or result table names
- Some parameters are restricted from agent-determined scope for security or consistency reasons

<Aside type="note">
Fixed and User input values do *not* appear in the agent's context—they are passed directly to the tool.
Agent-determined inputs are generated by the LLM during execution and appear in the agent's reasoning trace.
</Aside>

### Agent input schema

When tool parameters are configured as **User input**, they appear in the agent's Input section grouped by tool name. This makes it clear which parameters will be passed to which tools.

![Agent Input Configuration](../../../../assets/img/getting-started/agents/agent-inputs.png)

In the example above, `data_product_id` and `data_product_version` are labeled under "SQL Execution" to indicate they will be passed to that tool. This grouping helps users understand exactly what information they need to provide and where it will be used.
...
<Aside type="tip">
Before deploying, make sure your agent is **published**. Draft agents are only visible to you and admin roles. See [Publishing and unpublishing agents](/agent-studio-docs/security/identity-and-permissions/#publishing-and-unpublishing-agents-in-the-ui).
</Aside>
...
- Explore the [built-in agents](/agent-studio-docs/build/agents/default-agents/) to find a starting point to clone.
- Review [evaluations](/agent-studio-docs/build/evals/sql-evaluations/) to systematically test and tune agents.
- Learn about [flows](/agent-studio-docs/build/flows/overview/) for orchestration and automation at scale.
- To use your published agent from Claude, ChatGPT, or another client, [publish it to MCP](/agent-studio-docs/integrations/mcp/using-the-mcp-server/#publish-an-agent-to-mcp) and follow the [client setup guides](/agent-studio-docs/integrations/mcp-clients/claude/).
